...
export const isValidPassword = function (value, min_length) {
  if(!value || value === '' || value.length < min_length)    {return false;}
  return true;
};
...
export const itemIsChecked = function (desiredValue, itemValue) {
  ...
  if(Array.isArray(desiredValue))    {return desiredValue.indexOf(itemValue) >= 0 ? 'checked' : '';}
...
export const optionIsSelected = function (desiredValue, itemValue) {
  ...
  if(Array.isArray(desiredValue))    {return desiredValue.indexOf(itemValue) >= 0 ? 'selected' : '';}
      ...
      if(dataType == 'INTEGER') {
        if(fieldValue == '') {
          ...
          if(minValue && !isNaN(parseInt(minValue)) && intValue < parseInt(minValue)) {
            if(maxValue && !isNaN(parseInt(maxValue)))              {validationError(labelText + ' must be between ' + minValue + ' and ' + maxValue);}            else							{validationError(labelText + ' must be equal or greater than ' + minValue);}
            return false;
          }

          if(maxValue && !isNaN(parseInt(maxValue)) && intValue > parseInt(maxValue)) {
            if(minValue && !isNaN(parseInt(minValue)))              {validationError(labelText + ' must be between ' + minValue + ' and ' + maxValue);}            else							{validationError(labelText + ' must be equal or less than ' + maxValue);}
            return false;
          }
          fieldValue = intValue;
        }
